...
var Connection = function (optionArgs) {
	...
	this.sendNotificationUsingBuffer = function (note, bMessage) {
		if (!Buffer.isBuffer(bMessage)) {
			return Errors['message param should be utf-8 encoded buffer'];		
		}
		var encoding = 'utf8';
		if (note.encoding) {
			encoding = note.encoding;
		...
		var token = note.device.token;
		//var message = JSON.stringify(note);
		var messageLength = bMessage.length; //Buffer.byteLength(message, encoding);
		var pos = 0;
		...
		pos += int2buf(messageLength, data, pos, 2);
		//pos += data.write(message, pos, encoding);
		bMessage.copy(data, pos); //0, bMessage.length
		pos += messageLength;
